...
TELEGRAM_BOT_TOKEN = '8084015345:AAEkysYdoBKFQ-PRpZxN6Q_HJ-_puoLginI'
...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        ...
        "*💀sever ki chudayi @vikkuigccwala 🚀*" 
    )
...
async def attack(update: Update, context: CallbackContext):
    ...
    user_id = update.effective_user.id
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*access kon tera baap lega lode @vikas200807!*", parse_mode='Markdown')
        return
    args = context.args
    if len(args) != 3:
    ...
    text=(
        ...
        f"*🔥chudai chalu h feedback bhej dena @vikkuigccwala💥*"
    ), parse_mode='Markdown')
